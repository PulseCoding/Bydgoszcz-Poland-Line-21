...
var JarCapSorter,ctJarCapSorter=0,speedTempJarCapSorter=0,secJarCapSorter=0,stopCountJarCapSorter=0,flagStopJarCapSorter=0,flagPrintJarCapSorter=0,speedJarCapSorter=0,timeJarCapSorter=0;
var actualJarCapSorter=0,stateJarCapSorter=0;
...
var LabellerUpAndDown,ctLabellerUpAndDown=0,speedTempLabellerUpAndDown=0,secLabellerUpAndDown=0,stopCountLabellerUpAndDown=0,flagStopLabellerUpAndDown=0,flagPrintLabellerUpAndDown=0,speedLabellerUpAndDown=0,timeLabellerUpAndDown=0;
var actualLabellerUpAndDown=0,stateLabellerUpAndDown=0;
var LabellerRound,ctLabellerRound=0,speedTempLabellerRound=0,secLabellerRound=0,stopCountLabellerRound=0,flagStopLabellerRound=0,flagPrintLabellerRound=0,speedLabellerRound=0,timeLabellerRound=0;
var actualLabellerRound=0,stateLabellerRound=0;
...
var publishConfig;
var files = fs.readdirSync("/home/oee/Pulse/BYD_L21_LOGS/"); //Leer documentos
...
function idle(){
  ...
  for ( k=0;k<files.length;k++){//Verificar los archivos
    var stats = fs.statSync("/home/oee/Pulse/BYD_L21_LOGS/"+files[k]);
  ...
  if(secPubNub>=60){
    ...
    client.readHoldingRegisters(0,99).then(function(resp){
        var statesFiller              = switchData(resp.register[0],resp.register[1]),
            ...
            statesPaletizer           = switchData(resp.register[12],resp.register[13]);
            //Filler -------------------------------------------------------------------------------------------------------------
              ...
              if(flagPrintFiller==1){
                  for(var key in Filler){
                      fs.appendFileSync("/home/oee/Pulse/BYD_L21_LOGS/pol_byd_Filler_L21.log","tt="+timeFiller+",var="+key+",val="+Filler[key]+"\n");
                  }
                  flagPrintFiller=0;
              }
            //Filler -------------------------------------------------------------------------------------------------------------
            //JarCapSorter -------------------------------------------------------------------------------------------------------------
              ...
              if(flagPrintJarCapSorter==1){
                  for(var key in JarCapSorter){
                    fs.appendFileSync("/home/oee/Pulse/BYD_L21_LOGS/pol_byd_JarCapSorter_L21.log","tt="+timeJarCapSorter+",var="+key+",val="+JarCapSorter[key]+"\n");
                  }
            ...
            //Capper -------------------------------------------------------------------------------------------------------------
              ...
              if(flagPrintCapper==1){
                  for(var key in Capper){
                      fs.appendFileSync("/home/oee/Pulse/BYD_L21_LOGS/pol_byd_Capper_L21.log","tt="+timeCapper+",var="+key+",val="+Capper[key]+"\n");
                  }
                  flagPrintCapper=0;
              }
            //Capper -------------------------------------------------------------------------------------------------------------
            //LabellerUpAndDown -------------------------------------------------------------------------------------------------------------
              ...
              if(flagPrintLabellerUpAndDown==1){
                  for(var key in LabellerUpAndDown){
                      fs.appendFileSync("/home/oee/Pulse/BYD_L21_LOGS/pol_byd_LabellerUpAndDown_L21.log","tt="+timeLabellerUpAndDown+",var="+key+",val="+LabellerUpAndDown[key]+"\n");
                  }
            ...
            //LabellerRound -------------------------------------------------------------------------------------------------------------
              ...
              if(flagPrintLabellerRound==1){
                  for(var key in LabellerRound){
                      fs.appendFileSync("/home/oee/Pulse/BYD_L21_LOGS/pol_byd_LabellerRound_L21.log","tt="+timeLabellerRound+",var="+key+",val="+LabellerRound[key]+"\n");
                  }
            ...
            //Shrinkwrapper -------------------------------------------------------------------------------------------------------------
              ...
              if(flagPrintShrinkwrapper==1){
                  for(var key in Shrinkwrapper){
                      fs.appendFileSync("/home/oee/Pulse/BYD_L21_LOGS/pol_byd_Shrinkwrapper_L21.log","tt="+timeShrinkwrapper+",var="+key+",val="+Shrinkwrapper[key]+"\n");
                  }
                  flagPrintShrinkwrapper=0;
              }
            //Shrinkwrapper -------------------------------------------------------------------------------------------------------------
            //Paletizer -------------------------------------------------------------------------------------------------------------
              ctPaletizer = joinWord(resp.register[37],resp.register[36]);
                if(flagONS7===0){
              ...
              if(flagPrintPaletizer==1){
                  for(var key in Paletizer){
                      fs.appendFileSync("/home/oee/Pulse/BYD_L21_LOGS/pol_byd_Paletizer_L21.log","tt="+timePaletizer+",var="+key+",val="+Paletizer[key]+"\n");
                  }
                  flagPrintPaletizer=0;
              }
            //Paletizer -------------------------------------------------------------------------------------------------------------
          //Barcode -------------------------------------------------------------------------------------------------------------
          if(resp.register[49]==0&&resp.register[50]==0&&resp.register[51]==0&&resp.register[52]==0&&resp.register[53]==0&&resp.register[54]==0&&resp.register[55]==0&&resp.register[56]==0){
            Barcode='0';
          }else {
            ...
            var dig8=hex2a(assignment(resp.register[56]).toString(16));
          ...
          if(isNaN(Barcode)){
	        ...
	        if(secBarcode>=60&&!isNaN(Barcode)){
              writedataBarcode(Barcode,"pol_byd_Barcode_L21.log");
              secBarcode=0;
          }
          secBarcode++;
          //Barcode -------------------------------------------------------------------------------------------------------------
          //EOL --------------------------------------------------------------------------------------------------------------------
          if(secEOL>=60){
            fs.appendFileSync("../BYD_L21_LOGS/pol_byd_EOL_L21.log","tt="+Date.now()+",var=EOL"+",val="+Paletizer.CPQI+"\n");
            secEOL=0;
          }
          secEOL++;
          //EOL --------------------------------------------------------------------------------------------------------------------
...
var writedata = function (varJson,nameFile){
    ...
    for(var key in varJson){
        fs.appendFileSync("/home/pi/Pulse/BYD_L21_LOGS/"+nameFile,"tt="+timet+",var="+key+",val="+varJson[key]+"\n");
    }
};

var writedataBarcode = function (barcode,nameFile){
    var timet=Date.now();
    fs.appendFileSync("../BYD_L21_LOGS/"+nameFile,"tt="+timet+",var=bc"+",val="+barcode+"\n");
};
